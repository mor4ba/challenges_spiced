...
function calcResult() {
  if (
    wheel1.innerText === wheel2.innerText &&
    wheel2.innerText === wheel3.innerText
  ) {
    newResult = 100;
  } else if (
    wheel1.innerText !== wheel2.innerText &&
    wheel2.innerText !== wheel3.innerText &&
    wheel1.innerText !== wheel3.innerText
  )
    newResult = 0;
  else {
    newResult = 10;
...
spinButton.addEventListener("click", async () => {
  ...
  try {
    ...
    await Promise.all([wheel1.spin(), wheel2.spin(), wheel3.spin()]).then(
      () => {
        calcResult();
